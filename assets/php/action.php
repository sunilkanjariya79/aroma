...
if (isset($_GET['deleteuser'])) {
    if ($_GET['id'] == $_SESSION['userdata']['uid']) {
    ...
    } elseif ($_SESSION['userdata']['is_admin'] == 1) {
        if (deleteUser($_GET['id'])) {
            header('location:/aroma/');
        } else {
            echo "<script> alert('could not delete user')</script>";
...
if (isset($_GET['deletepost'])) {
    ...
    if (deletePost($post_id)) {
        ...
        echo "<script> alert('Error in Deleting Post')</script>";
        exit();
    }
...
if (isset($_GET['deletebook'])) {
    ...
    if (deleteBook($book_id)) {
        ...
        echo "<script> alert('Error in Deleting Book')</script>";
        exit();
    }

}
if (isset($_GET['deletereport'])) {
    $report_id = $_GET['deletereport'];
    if (deleteReport($report_id)) {
        header("location:/aroma/?adminpanel");
    } else {
        echo "<script> alert('Error in Deleting Report')</script>";
        exit();
    }
